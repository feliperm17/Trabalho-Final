...
for i in range(51, 950):
    img_path = os.path.join(dir, f'{i}.jpg')     # Completa o diretorio onde se encontra a imagem: /SoybeanSeeds/Broken/'i'.jpg
    if os.path.exists(img_path):                 # Verifica se o arquivo existe
        gray_img = imread(img_path, as_gray=1)   # Le a imagem e salva na variavel em escala de cinza    
        gray_img_u = img_as_ubyte(gray_img)      # Converte a imagem pra uint8
        hist_lbp = lbp(gray_img_u,               # Extração de caracteristicas lbp
                P=8,
                R=2,
                method='nri_uniform')
        hist_lbp = hist_lbp.astype("float")      # Converte para float
        hist_lbp /= (hist_lbp.sum()+ 1e-6)       # Normaliza o histograma
        hist_lbp = np.insert(hist_lbp, 0, 1)     # Adiciona a classe ao inicio do array
        hist_lpq = lpq(gray_img, mode='nh')      # Extração de caracteristicas lpq
        hist = np.concatenate((hist_lbp, hist_lpq)) # Concatena os dois histogramas de caracteristicas
        features_hist.append(hist)                  # adiciona o histograma a tabela de caracteristicas
...
for i in range(51, 1001):
    ...
    if os.path.exists(img_path):   
        ...
        hist_lbp = np.insert(hist_lbp, 0, 2) 
        hist_lpq = lpq(gray_img, mode='nh')
        hist = np.concatenate((hist_lbp, hist_lpq))
...
for i in range(51, 1001):
    ...
    if os.path.exists(img_path):   
        ...
        hist_lbp = hist_lbp.astype("float")
        hist_lbp /= (hist_lbp.sum()+ 1e-6)
        hist_lbp = np.insert(hist_lbp, 0, 3) 
        hist_lpq = lpq(gray_img, mode='nh')
        hist = np.concatenate((hist_lbp, hist_lpq))
        features_hist.append(hist)
...
for i in range(51, 1001):
    ...
    if os.path.exists(img_path):   
        ...
        hist_lbp = hist_lbp.astype("float")
        hist_lbp /= (hist_lbp.sum()+ 1e-6)
        hist_lbp = np.insert(hist_lbp, 0, 4) 
        hist_lpq = lpq(gray_img, mode='nh')
        hist = np.concatenate((hist_lbp, hist_lpq))
        features_hist.append(hist)
...
for i in range(51, 1001):
    ...
    if os.path.exists(img_path):   
        ...
        hist_lbp = hist_lbp.astype("float")
        hist_lbp /= (hist_lbp.sum()+ 1e-6)
        hist_lbp = np.insert(hist_lbp, 0, 5) 
        hist_lpq = lpq(gray_img, mode='nh')
        hist = np.concatenate((hist_lbp, hist_lpq))
        features_hist.append(hist)


df_features_hist = pd.DataFrame(features_hist)

df_features_hist.to_csv('./features.csv', index=False)

print(df_features_hist)
