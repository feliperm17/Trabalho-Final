...
# gray_img = imread('./SoybeanSeeds/Broken/51.jpg', as_gray=True)
# print (gray_img)
# lpq_hist = lpq(gray_img, mode='nh')
# print(lpq_hist)
...
for i in range(1, 50):
    ...
    hist = lpq(gray_img, mode='nh')
    hist = np.insert(hist, 0, 1)

    lpq_hist.append(hist)
...
df_lpq_hist_b = pd.DataFrame(lpq_hist)

df_lpq_hist_b.to_csv('./featureslpqtest.csv', index=False)
